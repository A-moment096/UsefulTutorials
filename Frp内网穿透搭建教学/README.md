...
? **Frp 官网**：[https://github.com/fatedier/frp](https://github.com/fatedier/frp)

---

## ? 安装与配置

> **?? 开始之前请确保** >**?? 一个有公网 IP 的服务器** >**?? 你需要实现内网穿透的服务器或电脑**
...
> Windows 系统下载 [frp_0.61.1_windows_amd64.zip](https://github.com/fatedier/frp/releases/download/v0.61.1/frp_0.61.1_windows_amd64.zip)
> Linux 系统 amd 架构下载 [frp_0.61.1_linux_amd64.tar.gz](https://github.com/fatedier/frp/releases/download/v0.61.1/frp_0.61.1_linux_amd64.tar.gz)
> Linux 系统 arm 架构下载 [frp_0.61.1_linux_arm64.tar.gz](https://github.com/fatedier/frp/releases/download/v0.61.1/frp_0.61.1_linux_arm64.tar.gz)
> 通常服务器都是 amd 架构，如果是树莓派或香橙派就是 arm 架构。

Linux 查看系统架构的指令： **`uname -m`**
...
**Windows 下载方式**：愣着干什么？去点击下载啊！下载完把 zip 解压出来，然后进入文件夹。
...
现在我们到你拥有公网 ip 的服务器
通常这个服务器系统都是 linux
如果是 windows 那就更简单，跟着点击修改一个配置文件即可。
...
我们需要修改的是 **`frps.toml`** 打开这个文件 (windows 用记事本啥的都行)
...
> 在文件目录里，点击上方的路径栏输入 cmd 后按回车打开命令窗口
...
这个通常是你的个人电脑，你的树莓派、香橙派或 NAS。

> **? 提示**：你的客户端也要下载相同版本的 frp 压缩包并且解压，过程和第一步一样，这里就不多说了
...
> **?? 注意**：是**frpc.toml**！是**FRPC**！不要搞错了！和刚才的服务端不一样那个是**frps.toml**。
...
Linux 通过指令进入程序窗口查看日志，如果出现报错，可以发给 AI 解答

### ? 4. AI！！！救我！！！！

遇到问题通通交给 AI 解决就好了，让他看你的**运行日志**和你的**配置文件**
...
更主要的原因是原项目已经废除了用.ini 格式来配置 frp 文件。
希望这个教程对你有帮助，如果喜欢请点个赞吧！
这个教程在我的 GitHub 也有同步发出，希望你能去点个`Star ?` ！
